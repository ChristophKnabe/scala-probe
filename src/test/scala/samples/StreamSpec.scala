...
/** Tests an internal implementation of the sieve of Eratosthenes. */
class StreamSpec extends AnyFunSpec with Matchers {

  /** Returns a Stream of all natural numbers starting from n ascending. */
  def from(n: Int): LazyList[Int] = n #:: from(n + 1)

  /** Returns a lazy Stream of all prime numbers in Int range. */
  ...
  describe("from") {
    it("must be only computed for first number") {
      from(10).toString mustBe "Stream(10, ?)"
      from(20).toString mustBe "Stream(20, ?)"
    }
